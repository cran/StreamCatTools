...
```{r get_watershed}
tictoc::tic()
ws <- lc_get_watershed(comid = 19334077)
tictoc::toc()
```

### Adding a HUC2 query parameter and using multi-threading speeds the process significantly
```{r get_lake_and_watershed_faster}
tictoc::tic()
...
### View lake with watershed
```{r view_lake_watershed}
m1 <- mapview::mapview(ws, 
                       col.regions = "blue", 
                       alpha.regions = 0.1,  
                       color = "blue",
                       alpha = 0.8,
